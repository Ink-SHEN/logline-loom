// agents/qa/llm.js — 「设计好的 LLM API 接口」（与 agents/screenwriter、agents/storyboard、agents/prompt-writer、agents/editor 的 llm.js 为同一份共享客户端的副本）。
// 按 agents/README.md 第二节，共享调用管线最终应落 tools/；tools/ 入库前各 Agent 文件夹先自包含一份，届时一并迁移。
...
// ⑤质检 的用法说明：chat() 把 messages 原样 JSON 序列化后发出去，所以 content 既可以是字符串
// （纯文本比对片约 red_lines），也可以是 OpenAI 视觉格式的分段数组
// （[{type:'text'},{type:'image_url',image_url:{url:'data:image/jpeg;base64,...'}}]）——
// agents/qa/vision.js 抽帧后就是走后者，本文件不需要为视觉另开一套客户端。
